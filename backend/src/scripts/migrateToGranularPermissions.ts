...
export async function previewMigration() {
  ...
  try {
    ...
    const existingPerms = new Set(
      existingPermsResult.rows.map(p => `${p.resource}:${p.action}`)
    )
    
    // Find which permissions will be added
    const newPermissions = GRANULAR_PERMISSIONS.filter(
      p => !existingPerms.has(`${p.resource}:${p.action}`)
    )
    ...
    const roleUpdates = rolesResult.rows.map(role => {
      const currentPerms = role.permissions.filter((p: any) => p.resource !== null)
      const newPermsForRole: string[] = []
      
      currentPerms.forEach((perm: any) => {
        const key = `${perm.resource}:${perm.action}`
        const mappedPerms = PERMISSION_MAPPING[key as keyof typeof PERMISSION_MAPPING]
        if (mappedPerms) {
...
export async function executeMigration() {
  ...
  try {
    ...
    for (const role of rolesResult.rows) {
      ...
      const newPermsToAdd: string[] = []
      
      // Apply mapping rules
      currentPermKeys.forEach((key: string) => {
        const mappedPerms = PERMISSION_MAPPING[key as keyof typeof PERMISSION_MAPPING]
        if (mappedPerms) {
          mappedPerms.forEach(newPerm => {
