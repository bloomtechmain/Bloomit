      ...
      for (const permKey of employeePermissions) {
        if (permissionIds[permKey]) {
          await client.query(
            ...
            [roles['Employee'], permissionIds[permKey]]
          )
        ...
        if (permissionIds[permKey]) {
          await client.query(
            `INSERT INTO role_permissions (role_id, permission_id)
             VALUES ($1, $2)
             ON CONFLICT (role_id, permission_id) DO NOTHING`,
            [roles['Manager'], permissionIds[permKey]]
          )
        ...
        if (permissionIds[permKey]) {
          await client.query(
            `INSERT INTO role_permissions (role_id, permission_id)
             VALUES ($1, $2)
             ON CONFLICT (role_id, permission_id) DO NOTHING`,
            [roles['Admin'], permissionIds[permKey]]
          )
        ...
        if (permissionIds[permKey]) {
          await client.query(
            `INSERT INTO role_permissions (role_id, permission_id)
             VALUES ($1, $2)
             ON CONFLICT (role_id, permission_id) DO NOTHING`,
            [roles['Super Admin'], permissionIds[permKey]]
          )
...
export default grantEmployeePortalPermissions
