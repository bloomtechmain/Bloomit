...
async function main() {
  ...
  try {
    ...
    const beforeCount = await pool.query('SELECT COUNT(*) as count FROM permissions')
    ...
    const afterCount = await pool.query('SELECT COUNT(*) as count FROM permissions')
    ...
    console.log('\n👥 Checking roles...')
    const employeeRoleCheck = await pool.query(
