...
import { addToPasswordHistory } from '../utils/passwordHistory'
...
export const onboardEmployee = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (send_email) {
      try {
        emailResult = await sendWelcomeEmail(email, temporaryPassword)
      } catch (emailError) {
    ...
    try {
      await addToPasswordHistory(userId, passwordHash)
    } catch (historyError) {
...
export const getFullEmployeeProfile = async (req: Request, res: Response) => {
  const { id } = req.params
...
export const suspendEmployee = async (req: Request, res: Response) => {
  const { id } = req.params
    ...
    await client.query(
      `UPDATE users 
       SET account_status = 'suspended', 
           ...
           status_reason = $2
       WHERE id = $3`,
      [currentUserId, reason, employee.user_id]
    )
    ...
    await client.query(
      `UPDATE employees 
       SET is_active = FALSE,
           ...
           suspended_reason = NULL
       WHERE id = $5`,
      [terminationDate, currentUserId, reason, purgeDate, id]
    )
