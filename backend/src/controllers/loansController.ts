  ...
  try {
    ...
    for (let i = 1; i <= total_installments; i++) {
      ...
      await req.dbClient!.query(
        ...
        [loan.id, i, dueDate.toISOString().split('T')[0], monthly_installment_amount, 'PENDING']
      )
  ...
  try {
    ...
    if (summary.interest_rate) {
      const amortizationSchedule = generateAmortizationSchedule(
        parseFloat(summary.loan_amount),
        parseFloat(summary.interest_rate),
        summary.total_installments,
        new Date(summary.start_date),
        parseFloat(summary.monthly_installment_amount)
      )
  ...
  try {
    const result = await req.dbClient!.query(
      `UPDATE loans
       ...
       WHERE id = $6
       RETURNING *`,
      [borrower_name, bank_name, loan_type, notes, status, id]
    )
  ...
  try {
    ...
    const result = await req.dbClient!.query(
      `UPDATE loan_installments
       ...
       RETURNING *`,
      [payment_date, amount_paid, paid_bank, payment_description, id, installment_number]
    )
  ...
  try {
    const result = await req.dbClient!.query(
      `UPDATE loan_installments
       ...
       RETURNING *`,
      [payment_date, amount_paid, paid_bank, payment_description, status, installmentId, id]
    )
