...
import { 
  sendPOCreatedEmail, 
  sendPOPendingApprovalEmail, 
  sendPOApprovedEmail, 
  sendPORejectedEmail 
} from '../utils/emailService'

// Generate next PO number
export const getNextPoNumber = async (req: Request, res: Response) => {
  try {
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      await client.query(
        ...
        [purchaseOrderId, item.quantity, item.description, item.unit_price, item.total, i]
      )
    }
    ...
    const itemsResult = await client.query(
      `SELECT * FROM purchase_order_items WHERE purchase_order_id = $1 ORDER BY line_order`,
      [purchaseOrderId]
    )
    ...
    setImmediate(async () => {
      try {
        ...
        if (requested_by_user_id) {
          const userResult = await pool.query('SELECT email FROM users WHERE id = $1', [requested_by_user_id])
          
          if (userResult.rows.length > 0) {
            const requestorEmail = userResult.rows[0].email
            
            // Send creation confirmation to requestor
            await sendPOCreatedEmail({
            ...
            if (adminEmails.length > 0) {
              ...
              await sendPOPendingApprovalEmail(adminEmails, {
                poNumber: purchaseOrder.po_number,
                requestedByName: purchaseOrder.requested_by_name,
                requestedByEmail: requestorEmail,
                vendorName: purchaseOrder.vendor_name,
    ...
    if (items && items.length > 0) {
      for (let i = 0; i < items.length; i++) {
        ...
        await client.query(
          ...
          [id, item.quantity, item.description, item.unit_price, item.total, i]
        )
      }
    }
...
export const approvePurchaseOrder = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      WHERE id = $4
      RETURNING *`,
      [approved_by_user_id, approved_by_name, approved_by_title || null, id]
    )
    ...
    setImmediate(async () => {
      try {
        ...
        const poDetailsResult = await pool.query(`
          SELECT po.*, 
                 v.vendor_name,
                 c.contract_name as project_name,
  ...
  try {
    ...
    if (req.file) {
      ...
      const result = await pool.query(
        ...
        WHERE id = $3
        RETURNING *`,
        [documentId, `/documents/${documentId}/download`, id]
      )
...
export const downloadReceipt = async (req: Request, res: Response) => {
  ...
  try {
    const poResult = await pool.query(
      'SELECT receipt_document_id, po_number FROM purchase_orders WHERE id = $1',
      [id]
    )
...
export const deletePurchaseOrder = async (req: Request, res: Response) => {
  ...
  try {
    // Check if PO exists
    const checkResult = await pool.query(
      'SELECT po_number, status FROM purchase_orders WHERE id = $1',
      [id]
    )
