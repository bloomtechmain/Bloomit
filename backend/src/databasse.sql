...
CREATE TABLE IF NOT EXISTS active_sessions (id SERIAL, user_id integer NOT NULL, session_token uuid NOT NULL DEFAULT gen_random_uuid(), expires_at timestamp without time zone NOT NULL, ip_address character varying(100), user_agent text, created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP);
...
CREATE TABLE IF NOT EXISTS purchase_order_items (id SERIAL, purchase_order_id integer, quantity integer NOT NULL, description text NOT NULL, unit_price numeric(12,2) NOT NULL, total numeric(12,2) NOT NULL, line_order integer DEFAULT 0, created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP);
