 ...
 */
export async function sendPOCreatedEmail(
  data: POEmailData
): Promise<{ success: boolean; error?: string }> {
...
© ${new Date().getFullYear()} Bloomtech ERP. All rights reserved.
  ...
  const results = await Promise.all(
    adminEmails.map(email => 
      sendEmail(email, `🔔 PO ${poNumber} - Approval Needed`, html, text)
    )
  )
 ...
 */
export async function sendPOApprovedEmail(
  data: POEmailData
): Promise<{ success: boolean; error?: string }> {
...
// ==================== PAYSLIP SIGNATURE CONFIRMATION EMAIL ====================

interface PayslipSignedData {
  employeeName: string
  employeeEmail: string
  payslipId: number
  month: number
 ...
 */
export async function sendPayslipSignedConfirmation(
  data: PayslipSignedData
): Promise<{ success: boolean; error?: string }> {
