...
exports.getEmployeeIdFromUserId = getEmployeeIdFromUserId;
exports.validateEmployeeAccess = validateEmployeeAccess;
exports.requireEmployeeRole = requireEmployeeRole;
exports.attachEmployeeId = attachEmployeeId;
exports.requireEmployeePortalPermission = requireEmployeePortalPermission;
exports.getEmployeeFromRequest = getEmployeeFromRequest;
...
async function getEmployeeIdFromUserId(userId) {
    try {
        const result = await db_1.pool.query('SELECT id FROM employees WHERE user_id = $1', [userId]);
 ...
 */
function validateEmployeeAccess(req, res, next) {
    ...
    const hasAdminAccess = req.user.roleNames.some(role => ['Super Admin', 'Admin', 'Manager'].includes(role));
    if (hasAdminAccess) {
 ...
 */
function requireEmployeeRole(req, res, next) {
    if (!req.user) {
        return res.status(401).json({
            error: 'unauthorized',
            message: 'Authentication required'
    ...
    const hasEmployeeRole = req.user.roleNames.some(role => ['Employee', 'Manager', 'Admin', 'Super Admin'].includes(role));
    if (!hasEmployeeRole) {
        return res.status(403).json({
            error: 'forbidden',
 ...
 */
function attachEmployeeId(req, res, next) {
    if (!req.user) {
        return res.status(401).json({
            error: 'unauthorized',
            message: 'Authentication required'
 ...
 */
function requireEmployeePortalPermission(action) {
    return (req, res, next) => {
        if (!req.user) {
            return res.status(401).json({
                error: 'unauthorized',
 ...
 */
async function getEmployeeFromRequest(req) {
    if (!req.user) {
        return null;
    }
    try {
