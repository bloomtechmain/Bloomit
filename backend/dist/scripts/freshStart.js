...
// ─── Helpers ─────────────────────────────────────────────────────────────────
async function runTenantSQL(schemaName) {
    const fullSql = fs_1.default.readFileSync(path_1.default.join(__dirname, '../databasse.sql'), 'utf-8');
...
async function createCompany(client, companyName, schemaName, adminName, adminEmail, adminPassword) {
    // 1. Tenant row
    const tenantResult = await client.query(`INSERT INTO public.tenants (name, schema_name) VALUES ($1, $2) RETURNING id`, [companyName, schemaName]);
  ...
  `);
    ...
    const userResult = await client.query(`INSERT INTO public.users (name, email, password_hash, tenant_id, source, password_must_change, account_status)
     ...
     RETURNING id`, [adminName, adminEmail, passwordHash, tenantId]);
...
async function freshStart() {
    ...
    try {
        ...
        const tenantRows = await client.query('SELECT schema_name FROM public.tenants');
        ...
        const pwHistExists = await client.query(`SELECT 1 FROM information_schema.tables
    ...
    `);
        ...
        for (const perm of ALL_PERMISSIONS) {
            await client.query(`INSERT INTO public.permissions (resource, action, description)
         VALUES ($1, $2, $3)
        ...
        const companyA = await createCompany(client, 'Company A', 'tenant_company_a', 'Admin A', 'admin@companya.com', 'CompanyA@123');
        // ── STEP 6: Create Company B ──────────────────────────────────────────────
        console.log('\n🏢  Step 6: Creating Company B...');
        const companyB = await createCompany(client, 'Company B', 'tenant_company_b', 'Admin B', 'admin@companyb.com', 'CompanyB@123');
        // ── Summary ───────────────────────────────────────────────────────────────
        console.log('\n══════════════════════════════════════════════════');
