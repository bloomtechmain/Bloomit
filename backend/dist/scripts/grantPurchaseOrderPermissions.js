...
async function main() {
    ...
    // Get all roles
    const rolesResult = await db_1.pool.query('SELECT id, name FROM roles');
    const roleMap = {};
    rolesResult.rows.forEach((role) => {
        roleMap[role.name] = role.id;
    });
    // Get all permission IDs
    const allPermissions = await db_1.pool.query('SELECT id, resource, action FROM permissions WHERE resource = $1', ['purchase_orders']);
    const permissionMap = {};
    ...
    const assignPermissions = async (roleName, permissions) => {
        ...
        for (const perm of permissions) {
           ...
           VALUES ($1, $2) 
           ON CONFLICT DO NOTHING`, [roleMap[roleName], permissionMap[perm]]);
                count++;
            }
            else {
