...
async function main() {
    ...
    for (const perm of permissions) {
       ...
       SET description = EXCLUDED.description`, [perm.resource, perm.action, perm.description]);
    }
    ...
    for (const role of roles) {
       ...
       RETURNING id`, [role.name, role.description, role.is_system_role]);
        roleIds[role.name] = result.rows[0].id;
    }
    console.log(`✅ ${roles.length} roles inserted/updated`);
    // Get all permission IDs
    const allPermissions = await db_1.pool.query('SELECT id, resource, action FROM permissions');
    const permissionMap = {};
    allPermissions.rows.forEach((p) => {
        permissionMap[`${p.resource}:${p.action}`] = p.id;
    });
    // Helper function to assign permissions
    const assignPermissions = async (roleName, permissions) => {
        // Delete existing permissions for this role
        await db_1.pool.query('DELETE FROM role_permissions WHERE role_id = $1', [roleIds[roleName]]);
        // Insert new permissions
        for (const perm of permissions) {
            if (permissionMap[perm]) {
                await db_1.pool.query(`INSERT INTO role_permissions (role_id, permission_id) 
           VALUES ($1, $2) 
           ON CONFLICT DO NOTHING`, [roleIds[roleName], permissionMap[perm]]);
            }
            else {
    ...
    const adminUpdate = await db_1.pool.query(`INSERT INTO user_roles (user_id, role_id)
     SELECT u.id, $1
     FROM users u
     WHERE u.email = 'admin@example.com' OR u.name = 'admin'
     ON CONFLICT (user_id, role_id) DO NOTHING
     RETURNING user_id`, [roleIds['Super Admin']]);
