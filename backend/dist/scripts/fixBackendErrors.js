...
async function fixBackendErrors() {
    ...
    `);
      ...
      `);
        ...
        const subPermissionIds = [];
        for (const perm of subPermissions) {
            const result = await client.query(`INSERT INTO permissions (resource, action)
         VALUES ($1, $2)
        ...
        const rolesResult = await client.query(`SELECT id, name FROM roles WHERE name IN ('Admin', 'Super Admin', 'Accounting')`);
...
exports.default = fixBackendErrors;
