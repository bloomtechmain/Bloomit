...
async function seedModulePermissions() {
    ...
    try {
        ...
        for (const perm of modulePermissions) {
            ...
            const existing = await client.query('SELECT id FROM permissions WHERE resource = $1 AND action = $2', [perm.resource, perm.action]);
            if (existing.rows.length === 0) {
                await client.query('INSERT INTO permissions (resource, action, description) VALUES ($1, $2, $3)', [perm.resource, perm.action, perm.description]);
...
exports.default = seedModulePermissions;
