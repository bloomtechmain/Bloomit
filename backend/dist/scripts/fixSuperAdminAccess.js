...
async function main() {
    ...
    try {
        ...
        const userCheck = await db_1.pool.query('SELECT id, email, name FROM users WHERE id = $1', [TARGET_USER_ID]);
        ...
        const roleCheck = await db_1.pool.query(`SELECT id FROM roles WHERE name = 'Super Admin'`);
        if (roleCheck.rows.length === 0) {
            console.log('   ⚠️  Super Admin role not found, creating it...');
            const roleCreate = await db_1.pool.query(`INSERT INTO roles (name, description, is_system_role) 
        ...
        const deleteResult = await db_1.pool.query('DELETE FROM user_roles WHERE user_id = $1', [TARGET_USER_ID]);
        ...
        await db_1.pool.query(`INSERT INTO user_roles (user_id, role_id) VALUES ($1, $2)`, [TARGET_USER_ID, superAdminRoleId]);
        ...
        const allPermissions = await db_1.pool.query('SELECT id, resource, action FROM permissions ORDER BY resource, action');
        ...
        await db_1.pool.query('DELETE FROM role_permissions WHERE role_id = $1', [superAdminRoleId]);
        ...
        for (const perm of allPermissions.rows) {
         ...
         VALUES ($1, $2) 
         ON CONFLICT DO NOTHING`, [superAdminRoleId, perm.id]);
            grantedCount++;
        }
        ...
        const verifyRole = await db_1.pool.query(`SELECT r.id, r.name, r.description 
       FROM user_roles ur 
       JOIN roles r ON ur.role_id = r.id 
       WHERE ur.user_id = $1`, [TARGET_USER_ID]);
        ...
        const verifyPerms = await db_1.pool.query(`SELECT COUNT(*) as count 
       FROM role_permissions rp 
       WHERE rp.role_id = $1`, [superAdminRoleId]);
