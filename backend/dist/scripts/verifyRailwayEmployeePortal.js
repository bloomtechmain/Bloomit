...
async function verifyRailwayEmployeePortal() {
    ...
    `);
        ...
        if (hasPayslipsTable) {
            tablesToCheck.push({ name: 'payslip_signatures', description: 'Payslip signatures', exists: false }, { name: 'payslip_signature_tokens', description: 'Signature tokens', exists: false });
        }
        // Check each table
