...
async function runTenantSQL(client, schemaName) {
    const sqlPath = path_1.default.join(__dirname, '../databasse.sql');
...
async function setup() {
    ...
    `);
        ...
        for (const p of ALL_PERMISSIONS) {
            await client.query(`INSERT INTO public.permissions (resource, action) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [p.resource, p.action]);
        }
        console.log(`   ✅ ${ALL_PERMISSIONS.length} permissions seeded`);
        // ── 5. Bloomtech tenant ───────────────────────────────────────────────────
        console.log(`📋 Step 5: Creating tenant "${COMPANY_NAME}"...`);
        const existingTenant = await client.query(`SELECT id FROM public.tenants WHERE schema_name = $1`, [SCHEMA_NAME]);
        let tenantId;
        if (existingTenant.rows.length > 0) {
            ...
            const tenantResult = await client.query(`INSERT INTO public.tenants (name, schema_name) VALUES ($1, $2) RETURNING id`, [COMPANY_NAME, SCHEMA_NAME]);
        ...
        const existingRole = await client.query(`SELECT id FROM public.roles WHERE name = 'Super Admin' AND tenant_id = $1`, [tenantId]);
        let roleId;
        if (existingRole.rows.length > 0) {
            ...
            const roleResult = await client.query(`INSERT INTO public.roles (name, description, is_system_role, tenant_id)
         VALUES ('Super Admin', 'Full access to all features', TRUE, $1) RETURNING id`, [tenantId]);
        ...
        for (const perm of allPerms.rows) {
            await client.query(`INSERT INTO public.role_permissions (role_id, permission_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [roleId, perm.id]);
        }
        console.log(`   ✅ ${allPerms.rows.length} permissions assigned to Super Admin`);
        // ── 8. Admin user ─────────────────────────────────────────────────────────
        console.log('📋 Step 8: Creating admin user...');
        const existingUser = await client.query(`SELECT id FROM public.users WHERE email = $1`, [ADMIN_EMAIL]);
        if (existingUser.rows.length > 0) {
            console.log(`   ✅ Admin user already exists`);
        }
        else {
            const passwordHash = await bcryptjs_1.default.hash(ADMIN_PASS, 10);
            const userResult = await client.query(`INSERT INTO public.users (name, email, password_hash, tenant_id, source, password_must_change, account_status)
         VALUES ($1, $2, $3, $4, 'marketing_site', FALSE, 'active') RETURNING id`, [ADMIN_NAME, ADMIN_EMAIL, passwordHash, tenantId]);
            const userId = userResult.rows[0].id;
            await client.query(`INSERT INTO public.user_roles (user_id, role_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [userId, roleId]);
