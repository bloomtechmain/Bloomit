...
const getAllPayslips = async (req, res) => {
    ...
    `;
        ...
        const result = await db_1.pool.query(dataQuery, params);
...
const getPayslipById = async (req, res) => {
    ...
    `;
        const payslipResult = await db_1.pool.query(payslipQuery, [id]);
    ...
    `;
        const signaturesResult = await db_1.pool.query(signaturesQuery, [id]);
        return res.status(200).json({
...
const createPayslip = async (req, res) => {
    ...
    try {
      ...
      WHERE id = $1
    `;
        const empResult = await db_1.pool.query(empQuery, [employee_id]);
...
const updatePayslip = async (req, res) => {
    ...
    try {
        ...
        const currentResult = await db_1.pool.query(currentQuery, [id]);
...
const deletePayslip = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.pool.query(deleteQuery, [id]);
...
const adminApprove = async (req, res) => {
    ...
    `, [id, userId, 'ADMIN', signatureHash, ipAddress]);
      ...
      `;
            const employeeResult = await db_1.pool.query(employeeQuery, [userId, id]);
...
const rejectPayslip = async (req, res) => {
    ...
    try {
      ...
      RETURNING *
    `;
        const result = await db_1.pool.query(updateQuery, [reason, id]);
        if (result.rows.length === 0) {
...
const getMonthlyReport = async (req, res) => {
    ...
    `;
        const result = await db_1.pool.query(query, [year, month]);
    ...
    `;
        const deptResult = await db_1.pool.query(deptQuery, [year, month]);
        return res.status(200).json({
...
const getEmployeePayrollHistory = async (req, res) => {
    ...
    `;
        const ytdResult = await db_1.pool.query(ytdQuery, [id, currentYear]);
        return res.status(200).json({
            payslips: result.rows,
