...
function calculateDepreciation(originalValue, salvageValue, usefulLife, purchaseDate, depreciationMethod) {
    ...
    if (depreciationMethod === 'STRAIGHT_LINE') {
        ...
        accumulatedDepreciation = Math.min(monthlyDepreciation * monthsOwned, originalValue - salvageValue);
    ...
    try {
        ...
        const assetsWithDepreciation = result.rows.map(asset => {
            if (asset.depreciation_method && asset.salvage_value !== null && asset.useful_life) {
                const calc = calculateDepreciation(Number(asset.value), Number(asset.salvage_value), Number(asset.useful_life), asset.purchase_date, asset.depreciation_method);
                return {
                    ...asset,
