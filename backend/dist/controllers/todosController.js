...
const shareTodo = async (req, res) => {
    try {
        ...
        const todoCheck = await req.dbClient.query('SELECT user_id FROM todos WHERE id = $1', [id]);
...
const unshareTodo = async (req, res) => {
    try {
        ...
        await req.dbClient.query('DELETE FROM todo_shares WHERE id = $1 AND todo_id = $2', [shareId, id]);
...
const getTodoShares = async (req, res) => {
    try {
        ...
        const result = await req.dbClient.query(`SELECT ts.id, ts.todo_id, ts.shared_with_user_id, ts.permission, ts.created_at,
