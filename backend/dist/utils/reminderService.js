...
exports.generateAutoReminders = generateAutoReminders;
exports.getEmployeeEmail = getEmployeeEmail;
 ...
 */
async function checkQuoteForReminder(quote, settings, client) {
    // Check if there's already a pending reminder for this quote
    const existingReminder = await client.query('SELECT reminder_id FROM quote_reminders WHERE quote_id = $1 AND reminder_status = $2', [quote.quote_id, 'PENDING']);
 ...
 */
async function createAutoReminder(quote, client) {
    const today = new Date();
    await client.query(`
 ...
 */
async function sendReminderEmailForQuote(quote, client) {
    try {
        // Get reminder email address from settings
        const emailSetting = await client.query("SELECT setting_value FROM application_settings WHERE setting_key = 'reminder_email_address'");
        ...
        if (quote.assigned_to) {
            const employeeResult = await client.query('SELECT name FROM employees WHERE id = $1', [quote.assigned_to]);
    ...
    `;
        await (0, emailService_1.sendEmail)(reminderEmail, subject, html);
...
async function getEmployeeEmail(employeeId) {
    try {
        const result = await db_1.pool.query('SELECT email FROM employees WHERE id = $1', [employeeId]);
