...
exports.generatePayslipToken = generatePayslipToken;
exports.validatePayslipToken = validatePayslipToken;
exports.markTokenAsUsed = markTokenAsUsed;
exports.revokePayslipToken = revokePayslipToken;
exports.getTokenByPayslipAndEmployee = getTokenByPayslipAndEmployee;
 ...
 */
async function generatePayslipToken(payslipId, employeeId, ipAddress, userAgent) {
    try {
        ...
        await db_1.pool.query(`UPDATE payslip_signature_tokens 
       SET is_used = true, used_at = NOW() 
       WHERE payslip_id = $1 AND employee_id = $2 AND is_used = false`, [payslipId, employeeId]);
        // Insert new token
        const query = `
 ...
 */
async function validatePayslipToken(token) {
    try {
        // Query for token
        const query = `
      SELECT 
      ...
      WHERE token = $1
    `;
        const result = await db_1.pool.query(query, [token]);
        ...
        if (now > expiresAt) {
            ...
            await db_1.pool.query(`UPDATE payslip_signature_tokens SET is_used = true WHERE id = $1`, [tokenData.id]);
            return {
                valid: false,
 ...
 */
async function markTokenAsUsed(token, ipAddress, userAgent) {
    try {
        const query = `
      ...
      RETURNING id
    `;
        const result = await db_1.pool.query(query, [token, ipAddress || null, userAgent || null]);
        if (result.rows.length === 0) {
 ...
 * @returns Success boolean
 */
async function revokePayslipToken(token) {
    try {
        const query = `
      UPDATE payslip_signature_tokens
      SET is_used = true, used_at = NOW()
 ...
 */
async function getTokenByPayslipAndEmployee(payslipId, employeeId) {
    try {
        const query = `
      SELECT 
        id,
      ...
      LIMIT 1
    `;
        const result = await db_1.pool.query(query, [payslipId, employeeId]);
        if (result.rows.length === 0) {
            return null;
        }
        const tokenData = result.rows[0];
...
async function cleanupExpiredTokens() {
    try {
        const result = await db_1.pool.query(`SELECT cleanup_expired_tokens()`);
